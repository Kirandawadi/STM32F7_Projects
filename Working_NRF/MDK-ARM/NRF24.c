...
uint8_t NRF24_ReadReg(uint8_t addr)
{
  uint8_t dt=0, cmd;
  CS_ON;
  HAL_SPI_TransmitReceive(&hspi5,&addr,&dt,1,1000);
  if (addr!=STATUS)//если адрес равен адрес регистра статус то и возварщаем его состояние
  {
    cmd=0xFF;
    HAL_SPI_TransmitReceive(&hspi5,&cmd,&dt,1,1000);
  }
  CS_OFF;
  return dt;
}
...
void NRF24_WriteReg(uint8_t addr, uint8_t dt)
{
  ...
  CS_ON;
  HAL_SPI_Transmit(&hspi5,&addr,1,1000);//отправим адрес в шину
  HAL_SPI_Transmit(&hspi5,&dt,1,1000);//отправим данные в шину
  CS_OFF;
}
//------------------------------------------------
void NRF24_ToggleFeatures(void)
{
  ...
  CS_ON;
  HAL_SPI_Transmit(&hspi5,dt,1,1000);
  DelayMicro(1);
  dt[0] = 0x73;
  HAL_SPI_Transmit(&hspi5,dt,1,1000);
  CS_OFF;
...
void NRF24_Read_Buf(uint8_t addr,uint8_t *pBuf,uint8_t bytes)
{
  CS_ON;
  HAL_SPI_Transmit(&hspi5,&addr,1,1000);//отправим адрес в шину
  HAL_SPI_Receive(&hspi5,pBuf,bytes,1000);//отправим данные в буфер
  CS_OFF;
}
//------------------------------------------------
void NRF24_Write_Buf(uint8_t addr,uint8_t *pBuf,uint8_t bytes)
{
  ...
  HAL_SPI_Transmit(&hspi5,&addr,1,1000);//отправим адрес в шину
  DelayMicro(1);
  HAL_SPI_Transmit(&hspi5,pBuf,bytes,1000);//отправим данные в буфер
  CS_OFF;
}
//------------------------------------------------
...
void NRF24L01_Receive(void)
{
  ...
  uint16_t dt=0;
	...
	{
			//sprintf(str1,"In\r\n");
			//HAL_UART_Transmit(&huart1,(uint8_t*)str1,strlen(str1),20);
	}
	status = NRF24_ReadReg(STATUS);
	sprintf(str1,"STATUS: 0x%X\r\n",status);
	HAL_UART_Transmit(&huart1,(uint8_t*)str1,strlen(str1),20);
	LED_TGL;
  DelayMicro(10);
  status = NRF24_ReadReg(STATUS);
